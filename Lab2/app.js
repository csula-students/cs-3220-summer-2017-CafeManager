...
class Cart {
    ...
    render () {
        ...
        for (var i = 0; i < deleteButtons.length; i ++) {
			...
			deleteBtn.addEventListener('click', () => {
        		...
        		fakeList.splice(parseInt(deleteBtn.dataset.id), 1);
				alert('You are deleting' + deleteBtn.dataset.id);
                 console.log(fakeList);
                 this.store.cartItems = fakeList;
                 this.render();
			});
 ...
 result += '<tr><td>' + list[i].name + '</td><td>' + list[i].price + '</td><td><button class="delete-button" data-id="'+i+'">Delete</button></td></tr>';    
    }
